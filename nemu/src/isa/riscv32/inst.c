...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("000000? ????? ????? 001 ????? 00100 11", slli   , I, R(rd) = src1 << (imm & 0x1F));// 在RV32I中，仅当shamt[5]=0时该指令合法 没有实现
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , I, R(rd) = (int32_t)src1 >> (src2 & 0x1F));
	INSTPAT("010000? ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (int32_t)src1 >> (imm & 0x1F));// 在RV32I中，仅当shamt[5]=0时该指令合法，还没有实现
	INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->pc + 4; s->dnpc = (src1 + imm) & ~1);// 若省略 rd，则默认为 x1没有实现
  INSTPAT("000000? ????? ????? 101 ????? 00100 11", srli   , I, uint8_t shamt = imm & 0x1F; R(rd) = src1 >> shamt);
