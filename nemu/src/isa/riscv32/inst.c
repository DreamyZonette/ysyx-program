...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(rd) = Mr(src1 + imm, 4));
	...
	INSTPAT("??????? ????? ????? 001 ????? 00100 11", slli   , I, R(rd) = src1 << (imm & 0x3f));// 在RV32I中，仅当shamt[5]=0时该指令合法 没有实现
	INSTPAT("??????? ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (int32_t)src1 >> (imm & 0x3f));// 在RV32I中，仅当shamt[5]=0时该指令合法，还没有实现
  ...
  /*未完成 */INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R, R(rd) = (int32_t)src1 / (int32_t)src2);
  ...
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bge    , B, if(src1 >= src2) s->dnpc = s->pc + imm);
