...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I, uint8_t shamt = imm & 0x1F; R(rd) = src1 << shamt);// 在RV32I中，仅当shamt[5]=0时该指令合法 没有实现
	...
	INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->pc + 4; s->dnpc = (src1 + imm) & ~1);// 若省略 rd，则默认为 x1没有实现
  ...
  INSTPAT("??????? ????? ????? 110 ????? 00100 11", ori    , I, R(rd) = src1 | imm);// NEW
  INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti   , I, R(rd) = ((int32_t)src1 < (int32_t)imm) ? 1 : 0);// NEW
  ...
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, {uint64_t product = (uint64_t)src1 * (uint64_t)src2; R(rd) = product >> 32;});// NEW
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu , R, { int64_t product = (int64_t)(int32_t)src1 * (uint64_t)(uint32_t)src2; R(rd) = product >> 32;});// NEW
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, R(rd) = (int32_t)src2 == 0 ? -1 : (int32_t)src1 / (int32_t)src2);
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , R, R(rd) = (uint32_t)src2 == 0 ? -1 : (uint32_t)src1 / (uint32_t)src2);
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R, R(rd) = (int32_t)src2 == 0 ? -1 : (int32_t)src1 % (int32_t)src2);
  INSTPAT("0000001 ????? ????? 111 ????? 01100 11", remu   , R, R(rd) = src2 == 0 ? -1 : src1 % src2);
