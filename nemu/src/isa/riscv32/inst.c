...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(rd) = Mr(src1 + imm, 2));
	...
	INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, if(rd == 0) rd = 1; R(rd) = s->pc + 4; s->dnpc = (src1 + imm) & ~1);// 若省略 rd，则默认为 x1没有实现

  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(src1 + imm, 1, src2));
	INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, 4, src2));
	INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, Mw(src1 + imm, 2, src2));

  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, if(rd == 0) rd = 1; R(rd) = s->pc + 4; s->dnpc = s->pc + imm);// 若省略 rd，则默认为 x1没有实现
