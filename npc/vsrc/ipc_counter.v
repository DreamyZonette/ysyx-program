    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            if (ebreak & wbu_valid) begin
                ...
                $display("\033[1;33mLSU Performance Counter: %16d spent %16d cycles busy account for %f\033[0m", lsu_performance_counter, lsu_cycles_counter, $itor(lsu_cycles_counter) / $itor(cycle_counter));
                ...
                $display("\033[1;94m[1;94m]EXU instructions:%16d account for %02f%% spent %16d cycles\033[0m", 
                        EXU_instructions, $itor(EXU_instructions) / $itor(ifu_performance_counter), exu_cycle_counter);
                $display("\033[1;94m[1;94m]CSR instructions:%16d account for %02f%% spent %16d cycles\033[0m", 
                        CSR_instructions, $itor(CSR_instructions) / $itor(ifu_performance_counter), csr_cycle_counter);
                $display("\033[1;94m[1;94m]LSU instructions:%16d account for %02f%% spent %16d cycles\033[0m", 
                        LSU_instructions, $itor(LSU_instructions) / $itor(ifu_performance_counter), lsu_cycle_counter);
                $display("\033[1;94m[1;94m]icache hit rate = %f AMAT = %f\033[0m",  
